...
import SavedGrid from "@/components/user-private/SavedGrid";
...
import SharedVenueVendorsList from "@/components/user-private/SharedVenueVendorsList";
import { NewLogo } from "@/components/home/Header";

const SavedVenueAndVendor = () => {
  const [gridType, setGridType] = useState<"grid" | "list">("grid");
  const [isOpen, toggleOpen] = useCycle(false, true);

  const [userSavedVenueList, setUserSavedVenueList] = useState([]);

  const getUserVenuesByList = async () => {
    ...
    try {
      const res = await axios.get(
        `/api/user-private-venues-vendors-listwise?userEmail=${localStorage.getItem("email")}`
      );
      // console.log(res.data.data, " data");
      setUserSavedVenueList(res.data.data);
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <section>
        <div className="flex-col overflow-hidden relative flex min-h-[600px] 3xl:min-h-[680px] max-lg:min-h-[340px] w-full items-center  pt-5  max-md:max-w-full max-md:px-5">
          ...
          <div className="relative 2xl:h-[70vh] h-[60vh] max-h-[48vh] flex w-full max-w-[93%] flex-col max-md:max-w-full">
            <div className="md:fixed xl:right-[3%] xl:w-[94%] md:w-[91%] md:right-[5%] z-[1000] justify-between items-center  backdrop-blur-[5px] bg-[#2E6AB3]/25 bg-opacity-10 self-stretch hidden md:flex gap-5 px-10 py-2.5 rounded-[40px] x-w-full max-md:flex-wrap max-md:px-5">
            <Link href="/">
            <NewLogo
              width={190}
              height={36}
        ...
        >
          <div>
            <div className="relative justify-between pt-14 lg:pt-0 items-stretch self-stretch  w-full gap-5  max-md:max-w-full max-md:flex-wrap max-md:pr-0">
              ...
              <div className="flex max-md:items-start max-md:flex-col justify-between max-md:text-sm items-center gap-2.5">
                <h3 className="text-gray-500 font-light text-base max-md:text-sm max-md:pt-3">
          ...
          <div className="transition-all duration-300 ease-in-out">
            {gridType === "grid" ? (
              <SavedGrid data={userSavedVenueList} />
            ) : (
              <SharedVenueVendorsList data={userSavedVenueList} />
          ...
          {/* <Pagination className="pt-4">
            <PaginationContent>
              ...
              {Array.from({
                length: Math.ceil(userSavedVenueList?.length / itemsPerPage),
              }).map((_, index) => (
              ...
              <PaginationItem>
                <PaginationNext
                 //  href="#"
                  ...
                  className={
                    endIndex >= userSavedVenueList.length ? "opacity-50" : ""
                  }
                />
              </PaginationItem>
            </PaginationContent>
...
export default SavedVenueAndVendor;
