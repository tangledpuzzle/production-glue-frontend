...
const VenueAdmin = () => {
  ...
  const [EntityListUserName, setEntityListUserName] = useState<string>("");
  const { venuesShared, reset } = useVenueShareStore();
  ...
  const handleSend = async (Email: string) => {
    if (EntityListUserName === "") {
      return;
    }
    const EntityListNameId = generateId() + "venue";
    let state: any = false;
    if (venuesShared.length === 0) {
    ...
    setSending(true);
    for (let i = 0; i < venuesShared.length; i++) {
      // console.log(i, " i ",state);

      state = await sendVenuesToUser(
        venuesShared[i],
        Email,
        EntityListNameId,
        EntityListUserName
    ...
    if (state) {
      toast.success("Venues shared successfully");
      setSending(false);
      reset();
      router.push("/admin/venue");
    }
                      ...
                      <Fragment key={i}>
                        <div className="flex max-md:gap-0">
                            ...
                            <div className="max-md:w-1/4 max-md:block hidden">
                              <button
                                onClick={() => {
                                  ...
                                  setSelectUserEmail(user?.email)
                                  console.log(user?.email)
                                  setOpen(true);
                                }}
                                className="flex 2xl:text-xl justify-center max-md:text-xs text-sm rounded-md items-center bg-[#2E6AB3] py-4 max-xs:py-0.5 max-xs:px-3 px-20 max-md:py-1.5 max-md:px-3 max-2xl:px-14 hover:opacity-80 text-white gap-3"
                              >
                                Send{" "}
                          ...
                          <Modal open={open} onClose={() => setOpen(false)}>
                            <div className="max-md:w-72 w-96 ">
                              <div className="mx-auto my-4 w-full">
                                ...
                                <Input
                                  onChange={(e) => {
                                    setEntityListUserName(e.target.value);
                                  }}
                              ...
                              <button
                                onClick={() => {
                                  handleSend(selectUserEmail);
                                }}
                                className="flex 2xl:text-xl justify-center max-md:text-xs text-sm rounded-md items-center bg-[#2E6AB3] py-2 max-xs:py-0.5 max-xs:px-3 px-20 max-md:py-1.5 max-md:px-3 max-2xl:px-14 hover:opacity-80 text-white gap-3"
                              >
                                Send{" "}
                              </button>
                            </div>
                          ...
                          <div className="max-md:hidden block">
                            <button
                              ...
                              disabled={sending}
                              className="flex justify-center max-md:text-xs text-sm rounded-md items-center bg-[#2E6AB3] py-4 px-20 max-md:py-1.5 max-md:px-3 max-xl:px-6 max-2xl:px-14 hover:opacity-80 text-white gap-3 disabled:opacity-50 disabled:cursor-not-allowed disabled:pointer-events-none"
                            >
...
async function sendVenuesToUser(
  Venue: any,
  userEmail: string,
  EntityListNameId: string,
  EntityListUserName: string
) {
  try {
    const res = await axios.post("/api/admin-share-venues", {
      Venue,
      userEmail,
      EntityListNameId,
      EntityListUserName,
    });
    // console.log(res.data , "res.data venues");
